...
class PatchclampSealTestUI(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.HoldingList.addItems(['-60 mV', '-50 mV', '-40 mV', '-30 mV'])
        ...
        self.stopandholdbutton.clicked.connect(lambda: self.hold())
    ...
    def updateLabels(self, curOut, voltOut):
        ...
        try:
            ...
            curFit = curData[int(maxCurIndex+1):int(maxCurIndex+1+points)]-0.5*(np.mean(curData[math.floor(0.15*sampPerCyc):math.floor(sampPerCyc/2)-2])+np.mean(curData[math.floor(0.65*sampPerCyc):sampPerCyc-2]))
